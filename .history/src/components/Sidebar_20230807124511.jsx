...
const Sidebar = () => {
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", width: "287px", height: "100vh" }}>
      {items.map((item) => (
        <Box sx={{ display: "flex", flexDirection: "column"}}>
          <Typography variant="subtitle2" color={blueGrey[200]}>{item.name}</Typography>
          {item.elements.map(i => <Typography variant="subtitle3" color={blueGrey[400]}>{i}</Typography>)}
        </Box>
      ))}
    </Box>
  );
